...
class NativeVoiceService {
    constructor(extensionPath, language, statusBar, pythonPath) {
    ...
    async checkPermission() {
        return new Promise((resolve) => {
            const process = (0, node_child_process_1.spawn)(this.pythonPath, [this.helperPath, 'check_permission']);
            let output = '';
            let errorOutput = '';
            ...
            process.on('close', (code) => {
                try {
                    ...
                    if (result.status === 'permission_granted') {
                        ...
                        vscode.window.showErrorMessage(`Microphone access denied for ${exe}. Go to System Settings > Privacy & Security > Microphone and enable the Python executable shown.`, 'Open System Settings').then(selection => {
    ...
    async recordAndTranscribe(duration = 5) {
        ...
        return new Promise((resolve) => {
            const process = (0, node_child_process_1.spawn)(this.pythonPath, [
                this.helperPath,
                'record',
                duration.toString(),
                this.language
                ...
                for (const line of lines) {
                    ...
                    try {
                        ...
                        if (result.status === 'recording' && lastStatus !== 'recording') {
                            ...
                            this.statusBar.update('error');
                            const exe = result.executable || this.pythonPath;
                            vscode.window.showErrorMessage(`Microphone access denied for ${exe}. Go to System Settings > Privacy & Security > Microphone and enable the Python executable shown.`);
                            resolve(null);
                            return;
                        }
    ...
    async checkDependencies() {
        return new Promise((resolve) => {
            const process = (0, node_child_process_1.spawn)(this.pythonPath, ['-c', 'import pyaudio, speech_recognition']);
            process.on('close', (code) => {
                resolve(code === 0);
            });
        });
...
async function activate(context) {
    ...
    const voiceService = new NativeVoiceService(context.extensionPath, language, statusBar, pythonPath);
