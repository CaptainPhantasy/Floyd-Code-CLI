...
class VoiceInputPanel {
  ...
  constructor(
    ...
    private showInterim: boolean
  ) {}

  /**
   * Show or create the voice input panel
   */
  public show(onText: (text: string) => void): void {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const continuous = config.get<boolean>('continuous', false);
  const showInterim = config.get<boolean>('showInterimResults', true);
  ...
  let voicePanel: VoiceInputPanel | undefined;
  ...
  const toggleCommand = vscode.commands.registerCommand(
    ...
    () => {
      if (!voicePanel) {
        voicePanel = new VoiceInputPanel(
  ...
  vscode.window.showInformationMessage(
    'FLOYD Voice Input ready! Press Cmd+Shift+V to start voice input.',
    'OK'
  );
}
