/**
 * Floyd SUGGESTED - Lean Core Prompt System
 *
 * Based on "Lean Core" architecture:
...
/**
 * SUGGESTED Settings (Lean Core Optimized)
 */
export const SUGGESTED_SETTINGS = {
	temperature: 1.0,      // GLM-4.7 optimal
	top_p: 0.95,           // GLM-4.7 optimal
	max_tokens: 8192,
	do_sample: true,
};
...
You are **${agentName}**, a coding assistant optimized for GLM-4.7-Flash.
...
- ask, yolo, plan, auto, dialogue, fuckit
...
Commit loop: ${BACKTICK}git_stage${BACKTICK} => ${BACKTICK}run${BACKTICK}(tests) => ${BACKTICK}verify${BACKTICK} => ${BACKTICK}git_commit${BACKTICK}
...
Only load detailed reference when needed:
- FORMAT.md (output formatting)
- TOOLS_50.md (complete tool reference)
- PATTERNS.md (common workflows)
- EXAMPLES.md (canonical walkthroughs)
`;

	// ============================================================================
	// TOOL REFERENCE (Condensed for always-on core)
	// ============================================================================
	const toolsRef = `
# 50 TOOLS — QUICK REFERENCE

| Category | Tools | Key Tool |
|----------|-------|----------|
| **File** | 7 | ${BACKTICK}edit_file${BACKTICK} — surgical edits |
| **Search** | 2 | ${BACKTICK}codebase_search${BACKTICK} — discovery |
...
| **System** | 3 | ${BACKTICK}run${BACKTICK} — execute commands |
...
**Model:** ${agentName} (GLM-4.7-Flash Optimized)
