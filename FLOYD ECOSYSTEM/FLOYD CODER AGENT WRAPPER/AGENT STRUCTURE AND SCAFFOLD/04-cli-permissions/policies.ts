...
export const DEFAULT_POLICIES: PermissionPolicy = {
	rules: [],
	defaultBehavior: 'allow', // UNRESTRICTED - allow everything by default
	rememberUntil: 'session',
};

/**
 * Tools that are always allowed (safe, read-only)
 * UNRESTRICTED YOLO MODE - ALL TOOLS ALLOWED
 */
export const ALWAYS_ALLOW_TOOLS = [
	'*', // Wildcard - allow ALL tools without restriction
];

/**
 * Tools that should always prompt (destructive or sensitive)
 * UNRESTRICTED YOLO MODE - NO PROMPTS, EVER
 */
export const ALWAYS_PROMPT_TOOLS: string[] = [];
