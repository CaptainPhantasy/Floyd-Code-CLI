...
export class CacheManager {
    ...
    async clear(tier) {
        ...
        try {
            const files = await fs.readdir(tierPath);
            for (const file of files) {
                await fs.unlink(path.join(tierPath, file));
            }
        }
        catch {
    ...
    async getStats() {
        ...
        for (const tier of this.tiers) {
            ...
            for (const entry of entries) {
                try {
                    const stat = await fs.stat(this.getEntryPath(tier, entry.key));
                    tierSize += stat.size;
                }
                catch { }
            }
            stats.totals.entries += entries.length;
            stats.totals.sizeBytes += tierSize;
            stats.tiers[tier] = {
                count: entries.length,
                sizeBytes: tierSize,
                sizeLimit: SIZE_LIMITS[tier],
                ttl: TTL_CONFIG[tier],
            };
        }
        return stats;
